...
app.get('/', (req, res) => {
    res.send('<link>/authenticate</link>')
})
...
// `localhost`.
    ...
    if (req.client.authorized) {
        res.send(`<message>Hello ${cert.subject.CN}, your certificate was issued by ${cert.issuer.CN}!</message>`)
...
// truthiness.
        ...
        res.status(403)
           .send(`<message>Sorry ${cert.subject.CN}, certificates from ${cert.issuer.CN} are not welcome here.</message>`)
// And last, they can come to us with no certificate at all:
    } else {
        res.status(401)
           .send(`<message>Sorry, but you need to provide a client certificate to continue.</message>`)
    }
})
